...
const Project = props => {
  ...
  (temp === props.id) ? temp = true : temp = false;

  // ((temp % 2) === 0)? temp = false: temp = true;

  let userBox = classNames({
    'users': true,
    'users-selected': temp
  })
  ...
  return (
    <div className={userBox} onSubmit={props.submitHandler} onClick={() => props.getProjectActions(props.id)}><div className="user-data">
